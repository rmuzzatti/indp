...
```{r}
naive_difference_evoked_df <- summary_male_eEPSC_df$percent_change_data %>%
  filter(category == "1") %>%
  mutate(
    naive_difference0to5_vs_25to30 = t25to30 - t0to5,
    naive_difference0to5_vs_20to25 = t20to25 - t0to5,
...
# none of the male data is normally distributed
```
...
```{r}
cold_difference_evoked_df <- summary_male_eEPSC_df$percent_change_data %>%
  filter(category == "2") %>%
  mutate(
    cold_difference0to5_vs_25to30 = t25to30 - t0to5,
    cold_difference0to5_vs_20to25 = t20to25 - t0to5,
...
# none of the male data is normally distributed except for the 0-15
```
...
```{r}
noise_difference_evoked_df <- summary_male_eEPSC_df$percent_change_data %>%
  filter(category == "3") %>%
  mutate(
    noise_difference0to5_vs_25to30 = t25to30 - t0to5,
    noise_difference0to5_vs_20to25 = t20to25 - t0to5,
...
t_test_male_naive <-perform_t_tests_for_summary_plot(
  data = summary_male_eEPSC_df$summary_data %>% filter(letter %in%time=max(30)), # need to filter for 30 minute cells
  test_category = 1,
  include_all_treatments = "no",
  list_of_treatments = c("Control"),
  current_type = "eEPSC",
