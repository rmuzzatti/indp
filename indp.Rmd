...
pruned_eEPSC_control_plots
```
CELL GROUPS:
```{r}
naive_female_long <- pruned_eEPSC_df$individual_cells %>% filter(letter %in% c("AA","AC", "AD", "AE", "AF", "AG", "AH", "AI", "AJ", "AK", "AL", "AM", "AN", "AO", "AP", "AQ", "AR", "AS", "AT", "AU", "AX", "AY", "CF", "CG", "CH"))

cold_female_long <- pruned_eEPSC_df$individual_cells %>% filter(letter %in% c("RM1", "RM2", "RM3", "RM4", "RM5", "RM6", "RM7", "RM8", "RM9"))

cold_female_short <- pruned_eEPSC_df$individual_cells %>% filter(letter %in% c("RM100", "RM101", "RM102", "RM103", "RM104"))
```

Plot Pruned Data of Cold:
```{r}
pruned_eEPSC_cold_plots <- plot_raw_current_data(
  data = pruned_eEPSC_df$individual_cells,
  plot_treatment = "Control",
  plot_category = 2,
  current_type = "eEPSC",
...
```{r eval}
t_test_df <- summary_eEPSC_df$summary_data
list_of_long_recordings <- unique(t_test_df$letter)

t_test_df <- summary_eEPSC_df$summary_data %>% filter(letter %in% c("RM1", "RM2", "RM3", "RM4", "RM5", "RM6", "RM7")) %>% filter(time <= 25)



t_test_female <- perform_t_tests_for_summary_plot(
  data = t_test_df %>% filter(sex == "Female"),
  test_category = 2,
