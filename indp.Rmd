...
ggsave(RM1_plot, path = here("Figures"), file = "RM1_plot.png", width = 7, height = 5, units = "in", dpi = 300)
```
...
```{r}
args(plot_summary_current_data)
...
female_PPR_df %>%
  ...
  stat_compare_means(
    method = "t.test",
    comparisons = list(c("1", "2")), # compare Naive vs Cold
    label = "p.format",
    label.y = 1.9,
    size = 4
  ) +
  ...
  theme(legend.position = "none")
#maybe median maybe mean
#do we want to display p value if not signfigant or jsut ns because the summary data can only have stars
```
