...
Prune female FULL (30 minute) CELLS:
```{r}
...
# all the long ones %>% filter(letter %in% c("LS1", "LS2", "LS3", "LS4", "LS5", "LS6", "LS7", "LS8", "LS9", "LS10", "RM1", "RM2", "RM3", "RM4", "RM6", "RM7", "RM8", "RM9", "RMNC1", "RMNC1", "RMNC3", "RMNC4", "AV", "AW", "KMC8", "KMCL6", "KMCL9", "KML12", "KML5", "KML9", "TW15", "TW17", "TW18", "TW19", "TW2", "TW20", "TW21", "TW3", "TW37", "TW38", "TW4", "TW4", "TW8", "TW9", "TW24", "TW25", "TW26", "TW27"))
```
...
all cells MALE summary eEPSC data:
```{r}
summary_male_eEPSC_df <- make_summary_EPSC_data(
  data = raw_eEPSC_df %>% filter(sex == "Male") %>% filter(treatment == "Control") %>% filter(synapses == "Glutamate"), # because there is some GABA male data just to be safe
  current_type = "eEPSC",
  save_output_as_RDS = "no",
  baseline_interval = "t0to5",
  ending_interval = "t25to30"
...
testing assumptions for female naive summary data
```{r}
...
testing assumptions for male naive summary data
```{r}
naive_difference_evoked_df <- summary_male_eEPSC_df$percent_change_data %>% filter(letter %in% c("AV", "AW", "KMC8", "KMCL6", "KMCL9", "KML12", "KML5", "KML9")) %>%
  filter(category == "1") %>%
  mutate(
    naive_difference0to5_vs_25to30 = t25to30 - t0to5,
    naive_difference0to5_vs_20to25 = t20to25 - t0to5,
...
# none of the male data is normally distributed except the last 5 min
```

testing assumptions for female cold summary data
```{r}
...
```
testing assumptions for male cold summary data
```{r}
cold_difference_evoked_df <- summary_male_eEPSC_df$percent_change_data %>% filter(letter %in% c("TW15", "TW17", "TW18", "TW19", "TW2", "TW20", "TW21", "TW3", "TW37", "TW38", "TW4", "TW4", "TW8", "TW9")) %>%
  filter(category == "2") %>%
  mutate(
    cold_difference0to5_vs_25to30 = t25to30 - t0to5,
    cold_difference0to5_vs_20to25 = t20to25 - t0to5,
...
shapiro.test(cold_difference_evoked_df$cold_difference0to5_vs_25to30)

shapiro.test(cold_difference_evoked_df$cold_difference0to5_vs_20to25)

shapiro.test(cold_difference_evoked_df$cold_difference0to5_vs_15to20)

shapiro.test(cold_difference_evoked_df$cold_difference0to5_vs_10to15)

shapiro.test(cold_difference_evoked_df$cold_difference0to5_vs_5to10)

# none of the male data is normally distributed except for the 10-15
```

testing assumptions for female noise summary data
```{r}
...
testing assumptions for male nosie summary data
```{r}
noise_difference_evoked_df <- summary_male_eEPSC_df$percent_change_data %>% filter(letter %in% c("TW24", "TW25", "TW26", "TW27")) %>%
  filter(category == "3") %>%
  mutate(
    noise_difference0to5_vs_25to30 = t25to30 - t0to5,
    noise_difference0to5_vs_20to25 = t20to25 - t0to5,
...
shapiro.test(noise_difference_evoked_df$noise_difference0to5_vs_25to30)

shapiro.test(noise_difference_evoked_df$noise_difference0to5_vs_20to25)

shapiro.test(noise_difference_evoked_df$noise_difference0to5_vs_15to20)

shapiro.test(noise_difference_evoked_df$noise_difference0to5_vs_10to15)

shapiro.test(noise_difference_evoked_df$noise_difference0to5_vs_5to10)
...
t_test_male_naive <-perform_t_tests_for_summary_plot(
  data = summary_male_eEPSC_df$summary_data %>% filter(letter %in% c("AV", "AW", "KMC8", "KMCL6", "KMCL9", "KML12", "KML5", "KML9")), # long naive males
  test_category = 1,
  include_all_treatments = "no",
  list_of_treatments = c("Control"),
  current_type = "eEPSC",
...
summary_male_eEPSC_df$summary_data %>% group_by(letter) %>% summarize(max=max(time))
```

t-test for cold males:
```{r}
t_test_male_cold <-perform_t_tests_for_summary_plot(
  data = summary_male_eEPSC_df$summary_data %>% filter(letter %in% c("TW15", "TW17", "TW18", "TW19", "TW2", "TW20", "TW21", "TW3", "TW37", "TW38", "TW4", "TW4", "TW8", "TW9")), # long cold males
  test_category = 2,
  include_all_treatments = "no",
  list_of_treatments = c("Control"),
  current_type = "eEPSC",
...
t_test_male_noise_full <-perform_t_tests_for_summary_plot(
  data = summary_male_eEPSC_df$summary_data %>% filter(letter %in% c("TW24", "TW25", "TW26", "TW27")),
  test_category = 3,
  include_all_treatments = "no",
  list_of_treatments = c("Control"),
  current_type = "eEPSC",
...
cold_summary_plot <- plot_summary_current_data(
  data = pruned_female_eEPSC_df$all_cells, # can I get both sexes on here if I pruned the data differently
  plot_category = 2,
  plot_treatment = "Control",
  current_type = "eEPSC",
  y_variable = "amplitude",
